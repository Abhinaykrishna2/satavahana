...
fn now_ist() -> DateTime<FixedOffset> {
...
const OPTIONS_SIGNALS_HEADER: &str = "signal_id,datetime,underlying,option_type,strike,expiry,\
...
const OPTIONS_TRADES_HEADER: &str = "trade_id,signal_id,datetime_entry,datetime_exit,\
...
impl OptionsJournal {
    ...
    pub fn log_trade(
        ...
        let row = format!(
            ...
            strike,
            expiry,
            csv_esc(&entry_ctx.strategy),
            entry_ctx.confidence,
            csv_esc(&entry_ctx.session),
            csv_esc(&entry_ctx.regime),
            entry_ctx.spot,
            entry_ctx.atm_iv_pct,
            entry_ctx.iv_rank,
            entry_ctx.pcr_oi,
            entry_ctx.pcr_vol,
            entry_ctx.max_pain,
            entry_ctx.net_gex,
            entry_ctx.days_to_expiry,
            entry_price,
            exit_price,
            lots,
            lot_size,
    ...
    pub fn write_session_summary(&self, open_position_count: usize) {
        ...
        lines.push(format!(
            "  Day P&L  : ₹{:+.2}  ({:+.1}%)",
            capital_change, capital_change_pct
        ));
        lines.push(format!(
            "  Progress to ₹50,000 : {:.1}%  ({} trades remaining approx)",
            progress_to_target,
        ...
        if !self.winning_pcr.is_empty() {
            let avg_winning_pcr =
                self.winning_pcr.iter().sum::<f64>() / self.winning_pcr.len() as f64;
            let avg_winning_iv =
                self.winning_iv_rank.iter().sum::<f64>() / self.winning_iv_rank.len() as f64;
            ...
            lines.push(format!(
                "  Avg IV Rank at winning entry: {:.1}%  (IV was cheap — confirms buy-low-IV edge)",
                avg_winning_iv
            ));
            if !self.winning_session.is_empty() {
                let mut session_count: std::collections::HashMap<&str, usize> =
                    std::collections::HashMap::new();
                for s in &self.winning_session {
                    *session_count.entry(s.as_str()).or_insert(0) += 1;
                }
                let best_session = session_count
                    .iter()
                    .max_by_key(|e| e.1)
                    .map(|(k, _)| *k)
                    .unwrap_or("—");
                lines.push(format!(
                    "  Best winning session       : {}  (consider focusing entries here)",
                    best_session
                ));
            }
            lines.push(String::new());
        ...
        if win_rate > 70.0 && total_trades >= 5 {
            lines.push(format!(
                "  ✓ Win rate > 70% — confidence threshold is well-calibrated"
            ));
        }
        if self.total_net_pnl < 0.0 {
            lines.push(format!(
                "  ⚠ Net day loss — review which strategies lost; tighten stops for those"
            ));
        }
        lines.push(format!(
            "  → Open options_trades.csv in Excel; pivot by `strategy` and `outcome`"
        ));
        lines.push(format!(
            "  → Filter wins: note the iv_rank, pcr_oi, session columns"
        ));
        lines.push(format!(
            "  → Strategies with < 40% win rate: consider disabling or raising min_confidence"
        ));
        lines.push(String::new());
...
const EQUITY_ALERTS_HEADER: &str = "alert_id,datetime,symbol,direction,\
...
impl EquityJournal {
    ...
    pub fn log_alert(
        ...
        self.total_alerts += 1;
        if net_spread > self.best_net_spread {
            self.best_net_spread = net_spread;
        }
